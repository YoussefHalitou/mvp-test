...
export function PlanningClient() {
    ...
    if (!mounted) {
        return <div className="flex h-full items-center justify-center bg-slate-50"><Loader2 className="h-7 w-7 animate-spin text-blue-500" /></div>;
    }
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg m-4" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4">
                            <div className="text-sm text-slate-500 bg-slate-50 rounded-lg px-3 py-2">
                                {parseValidDate(planModal.date) ? format(parseValidDate(planModal.date)!, 'EEEE, d. MMMM yyyy', { locale: de }) : 'Datum nicht verfügbar'}
                            </div>
                                            ...
                                            <div className="flex items-center gap-3 cursor-pointer flex-1" onClick={() => applyTemplate(t.id)}>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-slate-400">
                                                        {parseValidDate(t.created_at) ? format(parseValidDate(t.created_at)!, 'dd.MM.yyyy HH:mm', { locale: de }) : 'Datum nicht verfügbar'}
                                                    </div>
                                                </div>
                                            </div>
