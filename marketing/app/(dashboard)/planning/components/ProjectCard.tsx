...
export function ProjectCard({
    ...
    return (
        <div ref={setNodeRef} style={style} className={cn(
            "bg-white rounded-xl border shadow-sm overflow-hidden flex flex-col md:flex-row",
            isBesichtigung ? "border-violet-300" : "border-slate-200",
            isDragging && "opacity-50 relative z-20 bg-slate-50"
        )}>
            {/* LEFT: Project Info */}
            <div className={cn(
                "border-b md:border-b-0 md:border-r flex flex-col justify-between transition-all",
                isBesichtigung ? "bg-violet-50 border-violet-200" : "bg-slate-50 border-slate-200",
                compact ? "md:w-1/4 px-3 py-2" : "md:w-1/3 px-5 py-4"
            )}>
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        {isBesichtigung ? (
                            <span className="text-xs font-bold text-violet-600 bg-violet-100 border border-violet-200 px-2 py-0.5 rounded">
                                🔍 Besichtigung
                            </span>
                        ) : (
                    ...
                    {isBesichtigung ? (
                        <>
                            <h4 className={cn("font-bold leading-snug mb-1 text-violet-800", compact ? "text-base" : "text-xl")}>
                                {plan.project?.name || 'Besichtigung'}
                            </h4>
                            {!compact && plan.project && (
                                <div className="text-sm text-violet-600 flex items-start gap-1.5 mb-2">
                            ...
                            {!compact && plan.notes && (
                                <div className="bg-violet-100 text-violet-800 border border-violet-200 p-2.5 text-xs italic rounded-lg relative">
                                    <span className="absolute top-1 right-2 text-violet-400 font-serif text-xl">"</span>
                                    {plan.notes}
                                </div>
                            )}
                        </>
                        ...
                        <>
                            <div className="flex items-center gap-2 mb-1">
                                <h4 className={cn("font-bold text-slate-800 leading-snug", compact ? "text-base" : "text-xl")}>
                                    {plan.project?.name || 'Unbekannt'}
                                </h4>
                                {plan.project?.mannanzahl != null && (
                                    <span className="inline-flex items-center gap-1 text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200 px-2 py-0.5 rounded-full shrink-0">
                                        <Users className="h-3 w-3" />
                                        {plan.project.mannanzahl} Mann
                                    </span>
                                )}
                            </div>
                ...
                <div className={cn("flex items-center gap-2 border-t", isBesichtigung ? "border-violet-200" : "border-slate-200/60", compact ? "mt-2 pt-2" : "mt-4 pt-4")}>
